...
```
上诉代码创建了一个 Server 结构，然后在这个结构上调用 ListenAndServe 方法，这样服务器就能够启动了。  

**执行编译命令**：`go build` 编译生成二进制可执行文件（在当前目录以及$GOPATH/bin目录），它就是 ChitChat 应用的服务器。  
**执行启动命令**：`./chitchat`启动服务器。  
 
 或者  

**执行编译并启动命令**：`go run .`启动服务器。

如果数据库里面已经创建了 ChitChat 应用所需的数据库表，那么只需要访问 http://localhost:8080/ 并注册一个新账号，就可以使用自己的账号在论坛上发布新帖子了。
...
## 项目预览图
### 主页
 ![Image text](https://github.com/huangpengju/chitchat/tree/main/public/image/index.png?raw=true)
### 帖子详情页
![Image text](https://github.com/huangpengju/chitchat/tree/main/public/image/thread_details.png?raw=true)
### 注册页
![Image text](https://github.com/huangpengju/chitchat/tree/main/public/image/signup.png?raw=true)
### 登录页
![Image text](https://github.com/huangpengju/chitchat/tree/main/public/image/login.png?raw=true)
